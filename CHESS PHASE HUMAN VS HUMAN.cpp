...
int replayCount = 0;           
int currentMove = -1;   // current position in history
...
bool isCheck(Piece* board[8][8], COLOR kingColor)
{
   ...
   for (int r = 0; r < 8; r++)
       for (int c = 0; c < 8; c++)
          if (board[r][c] != nullptr && board[r][c]->color == kingColor && (board[r][c]->sym == 'K' || board[r][c]->sym == 'k'))
          {
                            kr = r;
                            kc = c;
          }
           ...
           for (int r = 0; r < 8; r++)
           {
               for (int c = 0; c < 8; c++)
               {
                   if (board[r][c] != nullptr && board[r][c]->color != kingColor && board[r][c]->isLegalMove != nullptr)
                   {
...
int main()
{
    ...
    do
    {
        ...
        saveGame(board, current.color);
       
        if (isCheckmate(board, current.color))
        {
            system("cls");
            box(14, 14, board);
        ...
        if (isStalemate(board, current.color))
        {
            system("cls");
            box(14, 14, board);
        ...
        if (isCheck(board, current.color))
        {
            gotoRowCol(60, 120);
            cout << "CHECK!";
